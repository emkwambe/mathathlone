...
function EducatorLogin({ next }: { next: string | null }) {
  ...
  // SSO handler — Supabase redirects to /auth/callback on success
  const handleSso = async (provider: 'google' | 'azure') => {
    setError('');
    setSsoLoading(provider);
    const { error: ssoError } = await signInWithProvider(provider);
    if (ssoError) {
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <div className="space-y-2 mb-5">
            <button
              ...
              className="w-full flex items-center justify-center gap-3 py-2.5 px-4 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition"
            >
              {ssoLoading === 'google' ? (
          ...
          <div className="relative mb-5">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="bg-white px-3 text-gray-400 uppercase tracking-wider">or sign in with email</span>
            </div>
          </div>
